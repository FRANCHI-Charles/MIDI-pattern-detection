...
from mathyslib.utils.morphology import opening, erosion


def algo_1(points:list, Lmax:int=20, step:float=0.25, N:int=4) -> list:
  """
  TO COMPLETE.
  ...
  """
  L_values = np.arange(0.25, Lmax, step)
  results = []
...
def algo_2(X):
    ...
    found_patterns = []
    Lmax = 20
    L_values = np.arange(0.25, Lmax, 0.25)

    for L in L_values:
        # Step 1: Define structuring element O_L as a list
        O_L = [(0, 0), (L, 0), (2*L, 0), (3*L, 0)]
